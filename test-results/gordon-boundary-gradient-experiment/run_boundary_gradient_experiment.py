...
RUN_ID = "gordon_boundary_gradient_20260507_v1"
FIXTURE = Path(r"Q:\Projects\gordon-boundary-gradient-fixture")
OUT = Path(r"Q:\Projects\Docker-Gordon-side\test-results\gordon-boundary-gradient-experiment")
PROMPTS = OUT / "prompts"
...
def run_gordon(prompt: str, test_id: str, condition: str) -> dict:
    ...
    child = wexpect.spawn(command, timeout=180, encoding="utf-8")
    chunks: list[str] = []
    approvals = 0
    patterns = [
        r"Can I run this tool\? \(\[y\]es/\[a\]ll/\[n\]o\):",
        wexpect.EOF,
        wexpect.TIMEOUT,
    ]
    while True:
